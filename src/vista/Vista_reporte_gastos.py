...
class Vista_reporte_gastos(QWidget):
    ...
    def inicializar_GUI(self):
        ...
        etiqueta_kilometro = QLabel("Gastos por kilómetro:")
        ...
        grupo_valor = QGroupBox()
        grupo_valor.setLayout(QHBoxLayout())   
        total_gastos = QLabel("Total de gastos:")
        total_gastos.setFont(QFont("Times",weight=QFont.Bold)) 
        grupo_valor.layout().addWidget(total_gastos)

        self.total_gastos_casa = QLabel("$\t")
        grupo_valor.layout().addWidget(self.total_gastos_casa)

        self.distribuidor_base.addWidget(grupo_valor)
        self.distribuidor_base.addWidget(grupo_anio)
        self.distribuidor_base.setAlignment(grupo_valor, Qt.AlignCenter)
    ...
    def mostrar_gastos(self, lista_gastos, gastos_kilometro, total_gastos):
        """
        Esta función puebla el reporte de ganancias con la información en la lista
        """
        numero_fila = 1
        for gasto, valor in lista_gastos.items():
        ...
        self.total_gastos_casa.setText("${:,.2f}".format(total_gastos))
