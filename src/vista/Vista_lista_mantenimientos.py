...
class Vista_lista_mantenimientos(QWidget):
    ...
    def mostrar_mantenimientos(self, mantenimientos):
        ...
        for mantenimiento in self.mantenimientos:
            ...
            boton_editar.clicked.connect(partial(self.mostrar_dialogo_editar_mantenimiento, mantenimiento["id"]))
    ...
    def mostrar_dialogo_editar_mantenimiento(self, id_mantenimiento):
        ...
        """
        buscar_mantenimiento = list(filter(lambda x: x["id"] == id_mantenimiento, self.mantenimientos))[0]
        dialogo = Dialogo_crear_mantenimiento(buscar_mantenimiento)
        dialogo.exec_()
        if dialogo.resultado == 1:
            self.interfaz.editar_mantenimiento(id_mantenimiento, dialogo.texto_nombre.text(),
                                               dialogo.texto_descripcion.text())
    ...
    def eliminar_mantenimiento(self, indice_mantenimiento):
        ...
        mensaje_confirmacion.setText(
            "¿Esta seguro de que desea eliminar este mantenimiento?\nRecuerde que esta acción es irreversible")
